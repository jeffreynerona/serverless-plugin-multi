...
function getServerlessCustomFile(filename) {
  const filepath = path.join(process.cwd(), filename);
  if (fs.existsSync(filepath)) return filepath;
  throw new Error(`Cannot find ${filename} on root directory`);
}
...
function iterateOverServices(serverless, options, func) {
  ...
  return promiseSeriesMap(serviceFolders, (folder) => {
    ...
    return Promise.all(results.map(([folder, config], index) => {
      const serviceName = path.basename(folder);
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          try {
            resolve(func(config, serviceName, folder));
          } catch(e) {
            reject(e);
          }
        }, index * 3000);
