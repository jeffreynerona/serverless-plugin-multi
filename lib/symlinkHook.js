...
function writeTempServerlessConfig(folder, name, service, config, customConfig, options) {
  ...
  const tmpYamlData = _.merge({
    custom: customConfig ? customConfig[options.stage || 'sit'] : service.custom,
...
const generateYamlAndSymlinks = (serverless, options) => {
  serverless.cli.log(`Generating serverless.yml and symlinks for service`);
  ...
  return serverless.yamlParser.parse(serviceFilePath)
    .then((primaryConfig) => {
      const customFile = _.get(serverless, 'service.custom.multi.custom', null);

      if (customFile) {
        serverless.cli.log(`Retrieving ${customFile} and adding it to serverless.yml`);
        const customFilePath = utils.getServerlessCustomFile(customFile);
        return Promise.all([primaryConfig, serverless.yamlParser.parse(customFilePath)]);
      }

      return Promise.all([primaryConfig, null]);
    })
    .then(([primaryConfig, customConfig]) => {
      return utils.iterateOverServices(serverless, options, (config, name, folder) => {
        serverless.cli.log(`Generating serverless.yml and symlinks for service ${name}`);
        serverless.cli.log(`Generating serverless.yml and symlinks for service ${folder}`);
        // make links from this service to the main service
        createSymLinks(mainService, folder);
        writeTempServerlessConfig(folder, name, primaryConfig, config, customConfig, options);
      });
    });
};
